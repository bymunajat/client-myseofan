...
$stmt = $pdo->prepare("SELECT * FROM pages WHERE slug = 'video-downloader' AND lang_code = 'en'");
$stmt->execute();
$dbPage = $stmt->fetch();
...
$t = [
    ...
    'heading' => $dbPage ? Translator::translate($dbPage['title'], $lang) : __("Download <span class='text-emerald-600 border-b-4 border-emerald-400/30'>Instagram Videos</span> Easily", $lang),
    'subtitle' => $dbPage ? Translator::translate(strip_tags($dbPage['content']), $lang) : __('Get any Instagram video in high resolution directly to your phone or computer.', $lang),
...
// Re-apply highlight style to heading if it was hardcoded or from DB
if (!$dbPage && $lang !== 'en') {
    ...
    // If from DB, we might want to manually highlight certain keywords if they exist
...
<body class="flex flex-col">
    ...
    <header class="sticky top-0 z-50 backdrop-blur-md bg-white/50 border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8 font-semibold text-gray-500">
                <?php
                function renderHeaderMenu_sub($items, $pageIdentifier, $currentSlug, $depth = 0)
                {
                    foreach ($items as $item):
                        $isActive = (strpos($item['final_url'], 'video.php') !== false);
                        ...
                        <div class="relative group">
                            ...
                            <?php if ($hasChildren): ?>
                                <div
                                    ...
                                    <?php renderHeaderMenu_sub($item['children'], $pageIdentifier, $currentSlug, $depth + 1); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php endforeach;
                }
                renderHeaderMenu_sub($headerItems, $pageIdentifier, '');
                ?>
                <select onchange="location.href = '?lang=' + this.value"
                    class="appearance-none bg-white border border-gray-200 text-gray-700 font-bold py-2 px-4 rounded-xl shadow-sm outline-none">
    ...
    <main class="container mx-auto px-4 py-20 flex-1 text-center font-bold">
        ...
        <div class="max-w-3xl mx-auto mb-20 fade-in">
            <!-- Tool Tabs -->
            <div
                class="flex flex-wrap justify-center gap-2 mb-8 p-2 bg-white/20 backdrop-blur-xl rounded-[2rem] border border-white/40 w-max mx-auto shadow-sm">
                <a href="video.php?lang=<?php echo $lang; ?>"
                    class="px-6 py-3 rounded-[1.5rem] bg-emerald-600 text-white shadow-lg transition-all">Video</a>
                <a href="reels.php?lang=<?php echo $lang; ?>"
                    class="px-6 py-3 rounded-[1.5rem] hover:bg-white/50 text-gray-700 transition-all">Reels</a>
                <a href="story.php?lang=<?php echo $lang; ?>"
                    class="px-6 py-3 rounded-[1.5rem] hover:bg-white/50 text-gray-700 transition-all">Story</a>
                <a href="highlights.php?lang=<?php echo $lang; ?>"
                    class="px-6 py-3 rounded-[1.5rem] hover:bg-white/50 text-gray-700 transition-all">Highlights</a>
            </div>
        ...
        <?php if ($dbPage && !empty($dbPage['content'])): ?>
            <div class="max-w-4xl mx-auto mt-24 text-left fade-in">
                <div
                    class="prose prose-emerald prose-xl max-w-none bg-white p-12 md:p-20 rounded-[3rem] shadow-xl border border-gray-100">
                ...
                <div class="col-span-2">
                    <div class="flex items-center gap-3 mb-8">
                        ...
                        <h4 class="text-3xl font-black tracking-tighter">
                            <?php echo htmlspecialchars($settings['site_name'] ?: 'MySeoFan'); ?></h4>
                    </div>
