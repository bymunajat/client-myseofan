...
if (!array_key_exists($active_lang, $supported_langs)) $active_lang = 'en';
...
if ($_curr_p < 1) $_curr_p = 1;
if ($_curr_p > $_total_p) $_curr_p = $_total_p;
...
<head>
    ...
    <style>
        ...
        .tab-active { background: white; color: #10b981; border-color: #10b981; font-weight: 700; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1); }
        .page-active { background: #10b981; color: white; border-color: #10b981; }
    </style>
</head>
<body class="flex">
    ...
    <main class="flex-1 min-h-screen">
        <header class="bg-white border-b border-gray-200 px-8 h-20 flex items-center justify-between sticky top-0 z-50">
            <div>
                ...
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Multi-Language Management | Page <?php echo $_curr_p; ?> of <?php echo $_total_p; ?></p>
            </div>
            <a href="?seed=1" class="text-sm bg-emerald-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-100 transition-all">Reset & Fix Database</a>
        </header>

        <div class="p-8">
            <?php if ($message): ?>
                <div class="bg-emerald-50 text-emerald-600 p-4 rounded-2xl mb-8 border border-emerald-100 flex items-center gap-3 font-medium">
            ...
            <div class="flex gap-2 mb-8 bg-gray-200/50 p-1.5 rounded-2xl overflow-x-auto">
                <?php foreach ($supported_langs as $code => $name): ?>
                    <a href="?lang=<?php echo $code; ?>" class="px-6 py-3 rounded-xl text-sm transition-all <?php echo $active_lang === $code ? 'tab-active' : 'text-gray-500 hover:text-gray-800'; ?>">
                        <?php echo $name; ?>
                    </a>
                <?php endforeach; ?>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-8 border-b border-gray-50 bg-gray-50/30 flex items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <a href="?lang=<?php echo $active_lang; ?>&p=<?php echo max(1, $_curr_p - 1); ?>" class="px-4 py-2 rounded-xl text-xs font-bold transition-all border <?php echo $_curr_p > 1 ? 'bg-white border-gray-200 text-emerald-600 hover:bg-emerald-50' : 'bg-gray-50 border-gray-100 text-gray-300 cursor-not-allowed'; ?>">Back</a>
                        <a href="?lang=<?php echo $active_lang; ?>&p=<?php echo min($_total_p, $_curr_p + 1); ?>" class="px-4 py-2 rounded-xl text-xs font-bold transition-all border <?php echo $_curr_p < $_total_p ? 'bg-white border-gray-200 text-emerald-600 hover:bg-emerald-50' : 'bg-gray-100 border-gray-100 text-gray-300 cursor-not-allowed'; ?>">Next</a>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <table class="w-full text-left">
                    <thead class="bg-gray-50/50">
                        <tr>
                            <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] w-1/4">Identifier Key</th>
                            <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Translated Value</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        <?php foreach ($keys as $key): ?>
                            <tr class="group hover:bg-gray-50/50 transition-all">
                                <td class="px-8 py-6">
                                    <code class="text-xs font-bold text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg"><?php echo $key; ?></code>
                                </td>
                                <td class="px-8 py-6">
                                    <form action="?lang=<?php echo $active_lang; ?>&p=<?php echo $_curr_p; ?>" method="POST" class="flex items-center gap-4">
                                        ...
                                        <div class="flex-1 relative">
                                            <textarea name="t_value" rows="1" class="w-full px-5 py-4 rounded-2xl border border-gray-100 bg-gray-50/50 focus:bg-white focus:border-emerald-500 outline-none text-sm transition-all resize-none shadow-sm" oninput='this.style.height = "";this.style.height = this.scrollHeight + "px"'><?php echo htmlspecialchars($current_translations[$key] ?? ''); ?></textarea>
                                        </div>
                                        <button type="submit" class="bg-emerald-600 text-white px-6 py-4 rounded-2xl hover:bg-emerald-700 font-bold shadow-lg shadow-emerald-100 transition-all">SAVE</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                ...
                <div class="p-8 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                    <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Page <?php echo $_curr_p; ?> of <?php echo $_total_p; ?></p>
                    <div class="flex gap-2">
                        <?php for ($i = 1; $i <= $_total_p; $i++): ?>
                            <a href="?lang=<?php echo $active_lang; ?>&p=<?php echo $i; ?>" class="w-10 h-10 flex items-center justify-center rounded-xl border text-xs font-bold transition-all <?php echo $i == $_curr_p ? 'page-active' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'; ?>"><?php echo $i; ?></a>
                        <?php endfor; ?>
                    </div>
                </div>
                <?php endif; ?>
